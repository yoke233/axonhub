...
func TestInboundStream_TextBeforeThinkingClosesTextBlock(t *testing.T) {
	...
	require.Equal(t, "content_block_delta", events[6].Type)
	require.Equal(t, int64(1), *events[6].Index)
	require.Equal(t, "signature_delta", *events[6].Delta.Type)
	require.NotEmpty(t, *events[6].Delta.Signature)

	require.Equal(t, "content_block_stop", events[7].Type)
	require.Equal(t, int64(1), *events[7].Index)

	require.Equal(t, "content_block_start", events[8].Type)
	require.Equal(t, int64(2), *events[8].Index)
	require.Equal(t, "text", events[8].ContentBlock.Type)

	require.Equal(t, "content_block_delta", events[9].Type)
	require.Equal(t, int64(2), *events[9].Index)
	require.Equal(t, "text_delta", *events[9].Delta.Type)
	require.Equal(t, "answer", *events[9].Delta.Text)

	require.Equal(t, "content_block_stop", events[10].Type)
	require.Equal(t, int64(2), *events[10].Index)
	require.Equal(t, "message_delta", events[11].Type)
	require.Equal(t, "message_stop", events[12].Type)
}
