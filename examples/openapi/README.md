...
| Query | 作用 |
|---|---|
| `apiKeyQuotaUsages(apiKeyId, key)` | 查询某把 API Key 中启用了额度（quota）的各 profile 的实时用量（`requestCount` / `totalTokens` / `totalCost` 及统计窗口）。`apiKeyId` 与 `key` 二选一。 |

> ⚠️ `apiKeyQuotaUsages` 需要 `read_api_keys` 权限，且只能查询调用方 service account **所属项目内**的 Key；跨项目或不存在的 Key 统一表现为查不到（不泄漏 Key 是否存在）。
>
...
- **同项目约束**: 所有 mutation 与 query 仅能作用于调用方 service account 所属的项目；跨项目的 `apiKeyID` / `key` / `templateID` 会被拒绝（查不到）。
